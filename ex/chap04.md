...
**表4.b**はユーザ-焼肉評価値行列である。各行はユーザ$$u$$を、各列はアイテム$$i$$を表す。行列の$$(u, i)$$成分は、ユーザ$$u$$がアイテム$$i$$に与えた評価値$$r_{u,i} \in \{-1, +1\}$$を表す。ただし、$$?$$となっている成分は欠損値であることを表す。このとき、次の問いに答えなさい。
...
\lambda_{2} =   0.921, \;\;\;\; \boldsymbol{v}_{2} = [-0.067,  0.695, -0.708,  0.112]^{\mathsf{T}} \\
\lambda_{3} =   0.343, \;\;\;\; \boldsymbol{v}_{3} = [ 0.512,  0.108, -0.076, -0.848]^{\mathsf{T}} \\
\lambda_{4} =  -0.168, \;\;\;\; \boldsymbol{v}_{4} = [-0.600, -0.500, -0.494, -0.382]^{\mathsf{T}}
$$
